...
const secret = "garbage"; //process.env.SECRET_KEY;
...
module.exports = {
	...
	registerUser: async (req, res) => {
		try {
			...
			if (potentialUser) {
				res
					.status(400)
					.json({ message: "That email already exists. Please log in." });
			} else {
	...
	findLoggedInUser: async (req, res) => {
		try {
			const decodedJwt = jsonWebToken.decode(req.cookies.userToken, {
				complete: true,
			});
